...
var [, , projectsPath = "projects", outputFile = "projects.ts"] = process.argv;
var cwd = process.cwd();
var importBack = "../".repeat(outputFile.split("/").length - 1);
var oldProjectPath = projectsPath;
var importProject = (project = "") => (0, import_path.join)(importBack, oldProjectPath, project);
projectsPath = (0, import_path.join)(cwd, projectsPath);
outputFile = (0, import_path.join)(cwd, outputFile);
console.log("==] Find projects...");
async function main() {
  const projects = await (0, import_promises.readdir)(projectsPath);
  const sourceFileData = await (0, import_promises.readFile)(outputFile, "utf-8");
  const testIndex = (e) => /\/\*([^\*]+)\*\//.test(e);
  ...
  const projectsDefinations = await Promise.all(projects.map(async (project) => {
    console.log(`==] Input project "${project}"`);
    const projectPath = (0, import_path.join)(projectsPath, project);
    const manifest = await (0, import_promises.readFile)((0, import_path.join)(projectPath, "manifest.yml"), "utf-8");
    const manifestObject = import_yaml.default.parse(manifest);
    const _a = manifestObject, { readme = null, screenshots = null } = _a, info = __objRest(_a, ["readme", "screenshots"]);
    const readmeOutput = {};
    ...
    if (readme) {
      ...
      for (const lang in readme)
        readmeOutput[lang] = `{{ import('${(0, import_path.join)(importPath, readme[lang])}?raw') }}`;
    }
    if (screenshots) {
      if (typeof screenshots != "string")
        throw new Error("Property screnshots need string!");
      const screenshotsDirectory = (0, import_path.join)(projectPath, screenshots);
      const screenshotsFiles = await (0, import_promises.readdir)(screenshotsDirectory);
      for (const file of screenshotsFiles) {
        screenshotsOutput.push({
          name: file,
          value: `{{ import('${(0, import_path.join)(importPath, screenshots, file)}?url') }}`
        });
      }
    }
    return JSON.stringify(__spreadProps(__spreadValues({
      id: project
    }, info), {
      screenshots: screenshotsOutput.length ? screenshotsOutput : null,
      readme: readmeOutput
    }), null, 2).replace(/("\{\{\s+|\s+\}\}")/g, "").replace(/"([^"]+)"\:/g, "$1:").replace(/"/g, `'`).replace(/null/g, "undefined");
  ...
  await (0, import_promises.writeFile)(outputFile, outputFileData);
}
